    ...
    <div id="job_manager">
        ...
        >
            <el-form :model="modifiedJobForm" label-width="120px">
                <el-form-item required="true" :label="$t('message.jobName')">
                ...
                <el-form-item required="true" :label="$t('message.executeConfig')">
                    <el-row>
                        <el-col :span="8">
                            <el-select
                ...
                <el-form-item required="true" label="处理器内容" v-if="!isContainer">
                    <el-row>
                        <el-col :span="24">
                            <el-input
                                type="textarea"
                    ...
                    <el-form-item>
                        ...
                        >
                            <el-option
                                ...
                                :key="worker.address"
                                :label="worker.address + '【' + worker.systemInfo + '】'"
...
export default {
    ...
    data() {
        return {
            ...
            processorTypeOptions: [
                ...
                { key: 'CONTAINER_SCRIPT', label: '脚本容器' },
                { key: 'SHELL', label: 'SHELL' },
                { key: 'PYTHON2', label: 'PYTHON2' },
                { key: 'PYTHON3', label: 'PYTHON3' },
                { key: 'JDK7', label: 'java -jar(JDK7)' },
                { key: 'JDK8', label: 'java -jar(JDK8)' },
                { key: 'JAVA_WINDOWS', label: 'java -jar(WINDOWS)'},
                { key: 'PYTHON_WINDOWS', label: 'python(WINDOWS)'},
                { key: 'EXE', label: 'exe(WINDOWS)'}
            ],
            // 执行方式类型
            executeTypeOptions: [
    ...
    methods: {
        ...
        saveJob() {
            const that = this;
            if(that.modifiedJobForm.designatedWorkers){
                this.modifiedJobForm.designatedWorkers= that.modifiedJobForm.designatedWorkers.toString();
            }
        ...
        onClickModify(data) {
            ...
            let workers=data.designatedWorkers.toString();
            
            if(workers != ''){
                let workerArr=workers.split(',');
                this.modifiedJobForm.designatedWorkers=workerArr
            }
            if(data.processorType == 'CONTAINER_SCRIPT'){
        ...
        onClickCopyJob(data) {
            ...
            let workers=res.designatedWorkers.toString();
            
            if(workers != ''){
                let workerArr=workers.split(',');
                that.modifiedJobForm.designatedWorkers=workerArr
            }
            if(res.processorType == 'CONTAINER_SCRIPT'){
                this.isContainer = true;
            }else{
                this.isContainer = false;
            }
        ...
        getContainer(row){
            this.modifiedJobForm.container = undefined;
            this.modifiedJobForm.processorInfo = undefined;
            if(row == 'CONTAINER_SCRIPT'){
                this.isContainer = true;
            }else{
                this.isContainer = false;
            }
        ...
        verifyPlaceholder(processorType) {
            ...
            switch (processorType) {
                ...
                case 'PYTHON':
                    ...
                    break;
                case 'PYTHON2':
                    res = this.$t('message.pythonFilePathPLH');
                    break;
                case 'PYTHON3':
                    res = this.$t('message.pythonFilePathPLH');
                    break;
                case 'JDK8':
                    res = this.$t('message.javaFilePathPLH');
                    break;
                case 'JDK7':
                    res = this.$t('message.javaFilePathPLH');
                    break;
                case 'JAVA_WINDOWS':
                    res = this.$t('message.javaWindowsPLH');
                    break;
                case 'PYTHON_WINDOWS':
                    res = this.$t('message.pythonWindowsPLH');
                    break;
                case 'EXE':
                    res = this.$t('message.exePLH');
                    break;
    ...
    mounted() {
        ...
        this.listJobInfos();
         ...
         that.axios.get('/system/listWorker?appId=' + this.$store.state.appInfo.id).then(res => {
            that.workerList = res;
        });

        that.axios.get('/container/list?appId=' + this.$store.state.appInfo.id).then(res => {
            
            that.containerScriptList = res.filter(item => {
                            return item.sourceType === 'Script';
                        });
             that.containerConfigList = res.filter(item => {
                            return item.sourceType === 'Config';
                        });            
